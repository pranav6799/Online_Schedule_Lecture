...
useEffect(() => {
  const fetchData = async () => {
    try {
      if (userId) {
        const response = await axios.get(`http://localhost:6001/api/v1/schedule/getUserSchedule/${userId}`);
...
useEffect(() => {
  ...
  return (
    ...
    </div>):(<div className='container text-center'><h1>No Lectures available.</h1></div>)
}
  </div>
  </div>
    </Layout>
